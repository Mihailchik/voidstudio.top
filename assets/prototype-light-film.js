// ПРОТОТИП светлой ленты. Копия filmstrip.js, не публикуется (.vercelignore).
// ?v=a — светлый фон, карточки притемнены как в тёмной ленте.
// ?v=b — светлый фон, карточки в покое выцветают к бумаге.
const LIGHT_VARIANT = new URLSearchParams(location.search).get('v') === 'a' ? 'a' : 'b';
const LIGHT_PAPER = '#efeee8', LIGHT_INK = '#1c1d1a';
...
function createFilmstrip(works, { reduced = false } = {}) {
  ...
  function changeScale(value) {
    ...
    if (!value) {
      syncVideoPlayback(-1);
      document.body.style.setProperty('--film-background', LIGHT_PAPER);
      document.body.style.setProperty('--film-ink', LIGHT_INK);
      document.body.style.setProperty('--film-title', LIGHT_INK);
